# -*- coding: utf-8 -*-
"""Antonio Gaglione Lab_2-Static Word Embeddings.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1qlpDQbynnYWIPOV8wKNDqkmqyr9zzNOo
...
for word in words_of_interest:
    similarities = [(k, embeddings.similarity(word, k)) for k in embeddings.index_to_key]

    # Converti i valori in un array NumPy di tipo float
    similarities = np.array(similarities, dtype=[('word', 'U50'), ('similarity', 'f4')])

    # Ordina in base alla similarità in ordine decrescente
    sorted_indices = np.argsort(-similarities['similarity'])

    # Seleziona le 10 parole più simili (escludendo la parola stessa)
    most_similar = [(similarities[i]['word'], similarities[i]['similarity']) for i in sorted_indices if similarities[i]['word'] != word][:10]

    DICT[word] = most_similar
...
for index, row in ws_df.iterrows():
    ...
    if word1 in embeddings and word2 in embeddings:
        ...
        similarities_list.append((word1, word2, similarity, row['similarity']))


similarities_df = pd.DataFrame(similarities_list, columns=['word1', 'word2', 'cosine_similarity', 'similarity'])


pearson_corr, pearson_pvalue = pearsonr(similarities_df['cosine_similarity'], similarities_df['similarity'])
spearman_corr, spearman_pvalue = spearmanr(similarities_df['cosine_similarity'], similarities_df['similarity'])
...
for word, similarity in word_vectors.most_similar(seed):
    if similarity >= threshold and word not in edges:
        edges.append((seed, word, similarity))

        for neighbor, neighbor_similarity in word_vectors.most_similar(word):
            if neighbor_similarity >= threshold and neighbor not in [edge[1] for edge in edges] and neighbor != 'pandemic':
                edges.append((word, neighbor, neighbor_similarity))
...
posts1 = covid_first_months['Post Description'].values.tolist()
posts2 = covid_last_months['Post Description'].values.tolist()

preprocessed_posts1 = []
for lemmatized_post in lemma.pipe(covid_first_months['Post Description']):
    preprocessed_posts1.append(" ".join(word.lemma_ for word in lemmatized_post if word.lemma_ not in stop_words))

preprocessed_posts2 = []
for lemmatized_post in lemma.pipe(covid_last_months['Post Description']):
    preprocessed_posts2.append(" ".join(word.lemma_ for word in lemmatized_post if word.lemma_ not in stop_words))

#print("ORIGINAL: ", posts1[0])
#rint("PROCESSED: ", preprocessed_posts1[0])

#print("ORIGINAL: ", posts2[0])
#print("PROCESSED: ", preprocessed_posts2[0])

covid_first_months.loc[:, "preprocessed_posts"] = preprocessed_posts1
covid_last_months.loc[:, "preprocessed_posts"] = preprocessed_posts2

preprocessed_posts1 = [post.split(" ") for post in preprocessed_posts1]
preprocessed_posts2 = [post.split(" ") for post in preprocessed_posts2]
...
model1 = Word2Vec(preprocessed_posts1, window=5, min_count=5, max_final_vocab=50000, vector_size=300, workers=1, seed=0)
word_vectors1 = model1.wv
model1.save("word2vec_covid_first_months_trained.model")
del model1

# train model on last months
model2 = Word2Vec(preprocessed_posts2, window=5, min_count=5, max_final_vocab=50000, vector_size=300, workers=1, seed=0)
word_vectors2 = model2.wv
model2.save("word2vec_covid_last_months_trained.model")
del model2
...
for word in vocabs:
    if word in word_vectors1.key_to_index:
        similar_words1[word] = [item[0] for item in word_vectors1.most_similar(positive=word, topn=50)]
    else:
        similar_words1[word] = []

    if word in word_vectors2.key_to_index:
        similar_words2[word] = [item[0] for item in word_vectors2.most_similar(positive=word, topn=50)]
    else:
        similar_words2[word] = []

common_words_count = {}
for word in vocabs:
    common_words_count[word] = len(set(similar_words1[word]).intersection(set(similar_words2[word])))


ten_most_changed = sorted(common_words_count, key=common_words_count.get)[:10]

#print("Words that changed the most:", ten_most_changed)
